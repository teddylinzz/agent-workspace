...
def grade_eval_1(output_path, timing_data):
    ...
    # Find the top 5 table or list
    # Let's search for lines containing stock IDs in a table format
    # Programmatic check for selects_5_stocks: count unique 4-digit numbers in table rows
    # Usually the table rows start with | and contain stock codes.
    table_rows = re.findall(r"\|\s*[^|]+\s*\|\s*(\d{4})\s*\|", content)
    # Filter to only keep rows before individual detail section or similar, or just check the number of recommendations.
    # In both reports, there is a summary table with 5 rows.
    # Let's extract the rows of the table.
    table_matches = []
    for line in content.split("\n"):
        if re.search(r"\|\s*(?:\d|\*\*?\d\*\*?)\s*\|\s*(\d{4})\s*\|", line):
            # Format: | 1 | 2344 | or | **1** | 2344 |
            stock = re.search(r"\|\s*(?:\d|\*\*?\d\*\*?)\s*\|\s*(\d{4})\s*\|", line).group(1)
            table_matches.append(stock)
        elif re.search(r"\|\s*1\s*\|\s*(\d{4})\s*\|", line) or re.search(r"\|\s*2\s*\|\s*(\d{4})\s*\|", line):
            # For table without bold indices
            stock = re.findall(r"\b(\d{4})\b", line)
            if stock:
                table_matches.append(stock[0])
    ...
    evidence_5_stocks = f"Found recommended stocks: {recommended} in the table." if selects_5_stocks else f"Found recommended stocks: {recommended} which is not exactly 5."
    ...
    # Verify entry, TP, and SL prices are provided
    includes_suggested_prices = "TP" in content and "SL" in content
    evidence_prices = "Found TP and SL references in the document." if includes_suggested_prices else "Did not find TP or SL references."
...
def grade_eval_2(output_path, timing_data):
    ...
    for line in content.split("\n"):
        if re.search(r"\|\s*(?:\d|\*\*?\d\*\*?)\s*\|\s*(\d{4})\s*\|", line):
            stock = re.search(r"\|\s*(?:\d|\*\*?\d\*\*?)\s*\|\s*(\d{4})\s*\|", line).group(1)
            table_matches.append(stock)
        elif re.search(r"\|\s*[1-5]\s*\|\s*(\d{4})\s*\|", line):
            stock = re.findall(r"\b(\d{4})\b", line)
            if stock:
                table_matches.append(stock[0])
...
for eval_name, config, grader in runs:
    dir_path = os.path.join(workspace_dir, eval_name, config)
    output_path = os.path.join(dir_path, "outputs", "watch_list_analysis.md")
    ...
    grading_json = {
        ...
        "eval_feedback": {
            "suggestions": [],
            "overall": "All assertions passed successfully."
        }
    }
