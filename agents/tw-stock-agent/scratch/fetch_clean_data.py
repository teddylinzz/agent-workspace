...
def main():
    ...
    # Load signals from paper_signals.json
    ...
    # Get the latest date
    dates = sorted(list(set(s.get("date") for s in all_signals if s.get("date"))))
    if not dates:
        print("No dates found in signals.")
        sys.exit(1)
    latest_date = dates[-1]
    print(f"Latest date in signals: {latest_date}")
    
    # Filter unique tickers for latest_date
    today_signals = [s for s in all_signals if s.get("date") == latest_date]
    unique_tickers = list(set(s["ticker"] for s in today_signals))
    print(f"Unique tickers for {latest_date}: {unique_tickers}")
    
    # Build unique mapping to avoid duplicates
    ticker_signals = {}
    ...
    out_path = '/Users/ted/agents/agents/tw-stock-agent/scratch/clean_candidates_details.json'
    with open(out_path, 'w', encoding='utf-8') as f:
        json.dump(output_data, f, ensure_ascii=False, indent=2)
    print(f"Saved {len(output_data)} records to {out_path}")
